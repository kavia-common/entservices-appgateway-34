...
| Proposed test case | Component(s) | Code paths / branches targeted | Setup pattern (based on reference) | Key assertions / observable outcomes |
|---|---|---|---|---|
| `Resolver_LoadConfig_MissingFile_ReturnsFalse` | `Resolver` | `Resolver::LoadConfig`: file open failure (`!file.is_open()`) | No mocks required; call `LoadConfig("/tmp/does-not-exist.json")` | Returns `false`. `IsConfigured()` remains `false`. |
| `Resolver_LoadConfig_InvalidJson_ReturnsFalse` | `Resolver` | `Resolver::LoadConfig`: parse failure (`!FromString(...)`) | Write invalid JSON to `/tmp/resolution.invalid.json` | Returns `false`. Does not add resolutions. |
| `Resolver_LoadConfig_MissingResolutionsObject_ReturnsFalse` | `Resolver` | `Resolver::LoadConfig`: `!config.Resolutions.IsSet()` branch | Write `{ "notResolutions": {} }` to `/tmp` | Returns `false`. |
| `Resolver_LoadConfig_LowercasesKeysAndOverrides` | `Resolver` | Key normalization (`StringUtils::toLower(it.Label())`), override behavior (`mResolutions[key] = ...`) | Write config with mixed-case key; optionally load two configs with same key to exercise override counter path | `ResolveAlias("MiXeDCaSe")` returns expected alias. Later-loaded config wins. |
| `Resolver_LoadConfig_EventAndComRpcFlags` | `Resolver` | `HasEvent(...)` uses `!event.empty()`. `HasComRpcRequestSupport(...)` uses `useComRpc` | Write config containing one entry with `event` set and another with `useComRpc: true` | `HasEvent(method)` true for event entry. `HasComRpcRequestSupport(method)` true for COM-RPC entry. |
| `AppGateway_Resolve_ResolverNotInitialized` | `AppGatewayImplementation` | `FetchResolvedData`: `mResolverPtr == nullptr` -> `CustomInitialize("Resolver not initialized")` | Create `AppGatewayImplementation` and call `Resolve(...)` before `Configure(IShell*)` or force resolver null | Returns `Core::ERROR_GENERAL`. `resolution` contains initialization error JSON. |
| `AppGateway_Resolve_ResolverNotConfigured` | `AppGatewayImplementation` | `FetchResolvedData`: `!mResolverPtr->IsConfigured()` -> `CustomInitialize("Resolver not configured")` | Configure plugin but ensure `LoadConfig` never succeeds (e.g., use minimal shell + default paths absent in test env), or explicitly clear resolutions if a hook becomes available | Returns `Core::ERROR_GENERAL`. `resolution` contains “Resolver not configured”. |
| `AppGateway_Resolve_MethodNotSupported_NoAlias` | `AppGatewayImplementation` | `ResolveAlias(method)` empty -> `ErrorUtils::NotSupported` | Use a resolver config that omits the tested method | Returns `Core::ERROR_GENERAL`. `resolution` contains NotSupported error JSON (validate by substring). |
| `AppGateway_Event_PreProcessEvent_MissingParams_BadRequest` | `AppGatewayImplementation` | `PreProcessEvent`: params JSON parse fails -> `CustomBadRequest("Event methods require parameters")` | Configure resolver so `HasEvent(method)` is true; call `Resolve(..., params="")` or invalid JSON | Returns `Core::ERROR_BAD_REQUEST`. `resolution` contains the “Event methods require parameters” message. |
| `AppGateway_Event_PreProcessEvent_MissingListen_BadRequest` | `AppGatewayImplementation` | `PreProcessEvent`: missing boolean `listen` -> `CustomBadRequest("Missing required boolean 'listen' parameter")` | Provide event method resolution; call with `{}` or `{ "listen": "true" }` | Returns `Core::ERROR_BAD_REQUEST`. Resolution contains message. |
| `AppGateway_Event_SubscribeListenTrue_ReturnsListeningResponse` | `AppGatewayImplementation` + `IAppNotifications` | `HandleEvent` calls `mAppNotifications->Subscribe(...)` and `PreProcessEvent` returns `{ "listening": true, "event": method }` | Reference Pattern 3: `ServiceMock` + `QueryInterfaceByCallsign<Exchange::IAppNotifications>(APP_NOTIFICATIONS_CALLSIGN)` returns mock; method configured as event | Asserts `Subscribe` called with `listen=true`, `alias`, and `event` equal to the method key passed into `Resolve`. Asserts resolution JSON includes `"listening":true` and `"event":"<method>"`. |
| `AppGateway_ComRpc_RequestHandlerMissing_NotAvailable` | `AppGatewayImplementation` + `IAppGatewayRequestHandler` | `ProcessComRpcRequest`: `QueryInterfaceByCallsign<...>(alias)` returns null -> `ErrorUtils::NotAvailable` | Configure resolver with `useComRpc: true` and an alias callsign; do not provide handler mock | Returns `Core::ERROR_GENERAL`. `resolution` contains NotAvailable error JSON. |
| `AppGateway_ComRpc_AdditionalContext_WrapsParamsWith_additionalContext` | `AppGatewayImplementation` + `IAppGatewayRequestHandler` | `UpdateContext(... onlyAdditionalContext=true)` path: wraps `{ "params": <orig>, "_additionalContext": <additionalContext + origin> }` | Provide resolver entry where `additionalContext` is an object and (by default) `useComRpc` becomes true; mock a request handler and inspect the `finalParams` string passed to `HandleAppGatewayRequest` | Asserts `finalParams` JSON contains `params` and `_additionalContext.origin == origin`, plus the configured additionalContext fields. |
| `AppGateway_PermissionGroup_Denied_NotPermitted` | `AppGatewayImplementation` + `IAppGatewayAuthenticator` | `HasPermissionGroup` branch; `SetupAppGatewayAuthenticator` + `CheckPermissionGroup(...)` yields `allowed=false` or error | Configure resolver with `permissionGroup`; return authenticator mock; test both error return and `allowed=false` | Returns `Core::ERROR_GENERAL`. `resolution` contains NotPermitted. |
| `AppGateway_PermissionGroup_AuthenticatorMissing_AllowsToProceed` | `AppGatewayImplementation` | Behavior when authenticator interface is unavailable: `SetupAppGatewayAuthenticator()` returns false and code does not fail closed | Configure resolver with `permissionGroup` but ensure `QueryInterfaceByCallsign<IAppGatewayAuthenticator>` returns null; ensure downstream path is reachable (e.g., COM-RPC handler mock exists) | Verifies the current behavior that missing authenticator does not deny access, and downstream handler is invoked. This is security-sensitive and should be locked down by test so changes are deliberate. |
