...
namespace ProxyStubs {
    ...
    ProxyStub::MethodHandler CompositPluginStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            const Core::instance_id parameter_62771d5cImplementation = reader.Number<Core::instance_id>();

            ICompositPlugin::INotification* _parameter_62771d5c = nullptr;
            ProxyStub::UnknownProxy* parameter_62771d5cProxy = nullptr;
            if (parameter_62771d5cImplementation != 0) {
                parameter_62771d5cProxy = RPC::Administrator::Instance().ProxyInstance(channel, parameter_62771d5cImplementation, false, _parameter_62771d5c);

                ASSERT((_parameter_62771d5c != nullptr) && (parameter_62771d5cProxy != nullptr));
            }

            uint32_t result = implementation->Register(_parameter_62771d5c);

            RPC::Data::Frame::Writer writer(message->Response().Writer());
            writer.Number<uint32_t>(result);

            if (parameter_62771d5cProxy != nullptr) {
                RPC::Administrator::Instance().Release(parameter_62771d5cProxy, message->Response());
            }
        },
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            const Core::instance_id parameter_8b1e535fImplementation = reader.Number<Core::instance_id>();

            ICompositPlugin::INotification* _parameter_8b1e535f = nullptr;
            ProxyStub::UnknownProxy* parameter_8b1e535fProxy = nullptr;
            if (parameter_8b1e535fImplementation != 0) {
                parameter_8b1e535fProxy = RPC::Administrator::Instance().ProxyInstance(channel, parameter_8b1e535fImplementation, false, _parameter_8b1e535f);

                ASSERT((_parameter_8b1e535f != nullptr) && (parameter_8b1e535fProxy != nullptr));
            }

            uint32_t result = implementation->Unregister(_parameter_8b1e535f);

            RPC::Data::Frame::Writer writer(message->Response().Writer());
            writer.Number<uint32_t>(result);

            if (parameter_8b1e535fProxy != nullptr) {
                RPC::Administrator::Instance().Release(parameter_8b1e535fProxy, message->Response());
            }
        }
        , nullptr
    ...
    public:
        ...
        uint32_t Register(ICompositPlugin::INotification* _parameter_62771d5c) override
        {
            IPCMessage message(BaseClass::Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_62771d5c));
        ...
        uint32_t Unregister(ICompositPlugin::INotification* _parameter_8b1e535f) override
        {
            IPCMessage message(BaseClass::Message(1));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_8b1e535f));
